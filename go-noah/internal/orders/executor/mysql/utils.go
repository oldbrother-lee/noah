...
// GetConnectionID 获取MySQL连接ID
func GetConnectionID(db *sql.DB) (int64, error) {
	var connectionID int64
	err := db.QueryRow("SELECT CONNECTION_ID()").Scan(&connectionID)
	if err != nil {
		return 0, fmt.Errorf("获取Connection ID失败: %w", err)
	}
	return connectionID, nil
}

// GetBinlogPos 获取MySQL binlog position
func GetBinlogPos(db *sql.DB) (file string, position int64, err error) {
	// 使用 Query 获取所有列，然后通过列名提取需要的值（兼容不同 MySQL 版本的列数差异）
	rows, err := db.Query("SHOW MASTER STATUS")
	if err != nil {
		return "", 0, fmt.Errorf("获取MySQL position失败: %w", err)
	}
	defer rows.Close()

	if !rows.Next() {
		return "", 0, errors.New("获取MySQL position失败: 未找到数据，请检查MySQL是否开启了binlog")
	}
	...
	for i, colName := range columns {
		...
		case "Position":
			switch v := values[i].(type) {
			case []byte:
				positionStr = string(v)
			case string:
				positionStr = v
			case int64:
				positionStr = strconv.FormatInt(v, 10)
			case int:
				positionStr = strconv.Itoa(v)
			default:
				positionStr = fmt.Sprintf("%v", v)
			}
		}
	}

	if fileStr == "" || positionStr == "" {
		return "", 0, errors.New("获取MySQL position失败: 未找到 File 或 Position 列")
	}

	position, err = strconv.ParseInt(positionStr, 10, 64)
	if err != nil {
		return "", 0, fmt.Errorf("解析position失败: %w", err)
	}

	return fileStr, position, nil
}

// isGenerated 判断是否为计算列
