...
func (c *ColOptions) CheckColumnNotAllowedType() error {
	...
	// 检查 BLOB/TEXT 类型：在 TiDB parser 中，TEXT 类型和 BLOB 类型使用相同的类型常量
	// TypeBlob 对应 BLOB 和 TEXT，TypeTinyBlob 对应 TINYBLOB 和 TINYTEXT
	// TypeMediumBlob 对应 MEDIUMBLOB 和 MEDIUMTEXT，TypeLongBlob 对应 LONGBLOB 和 LONGTEXT
	if !c.InspectParams.ENABLE_COLUMN_BLOB_TYPE && (c.Tp == mysql.TypeTinyBlob || c.Tp == mysql.TypeMediumBlob || c.Tp == mysql.TypeLongBlob || c.Tp == mysql.TypeBlob) {
		return fmt.Errorf("列`%s`不允许定义BLOB/TEXT类型[表`%s`]", c.Column, c.Table)
	}
